from .pose_merger_includes import *
...
def node_init(self):
    ...
    self.odom_sub = self.create_subscription(Odometry, '/iBot/odometry/filtered', self.odom_callback, 10)
    ...
    self.merged_pose_pub = self.create_publisher(Vector3, '/iBot/merged_pose', 10)

    # SERVICES

    self.save_pose_data_srv = self.create_service(Trigger, '/iBot/save_pose_data', self.save_pose_data_callback)
...
def save_pose_data_callback(self, request, response):
    d = {
        'truth_pose_x': truth_array[0][:1000],
        'truth_pose_y': truth_array[1][:1000],
        'ekf_pose_x': ekf_array[0][:1000],
        'ekf_pose_y': ekf_array[1][:1000],
        'merged_pose_x': merged_array[0][:1000],
        'merged_pose_y': merged_array[1][:1000],
        }
